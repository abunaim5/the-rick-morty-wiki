...
const MeetCastCard = ({ character }: { character: CharacterType }) => {
    ...
    return (
        <div onClick={() => router.push(`/cast/${character.id}`)} className={`relative w-full max-w-md clip-pentagon overflow-hidden ${location === '/casts' ? 'cursor-pointer' : 'pointer-events-none'}`}>
            <div className="bg-gradient-to-br from-[#84F729]/35 to-[#15BFFD]/35 p-[1px] rounded-lg clip-pentagon">
                <div className="w-full p-4 bg-[#1e1f2f]/80 backdrop-blur-sm rounded-lg clip-pentagon font-medium text-white flex flex-col items-start">
